	...
	public class PersistentQueueTests : PersistentQueueTestsBase
	{
		...
		public void Only_single_instance_of_queue_can_exists_at_any_one_time()
		{
			using (new PersistentQueue(path))
			{
                ...
                Assert.Throws<InvalidOperationException>(() => new PersistentQueue(path), "Another instance of the queue is already in action, or directory does not exists");
        ...
        public void Corrupt_index_file_should_throw()
        {
            ...
            // ReSharper disable once ObjectCreationAsStatement
            Assert.Throws<InvalidOperationException>(() => new PersistentQueue(path), "Unexpected data in transaction log. Expected to get transaction separator but got unknown data. Tx #1");
